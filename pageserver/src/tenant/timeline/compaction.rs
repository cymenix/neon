...
impl Timeline {
    ...
    async fn compact_shard_ancestors(
        ...
        for layer in layers_to_rewrite {
            ...
            // - We hold the `ResidentLayer` for the old layer file across our rename() of the new layer in its place.  That
            //   means that it's impossible for eviction/promotion to accidentally over-write our replacement layer while
            //   promoting from remote storage.
            //
            // If we make a mistake with any of the above, the stakes should be pretty low: as long as our new layer
            // lands in remote storage + index before we remove the old layer from remote storage + index, then everything
            // should still be readable after a restart (if e.g. we had a bug that hung readers of rewritten layers).
            let resident = layer.download_and_keep_resident().await?;
            let keys = resident
                .load_image_keys(ctx)
                .await
                .context(format!("Reading keys from {layer}"))?;
            let mut written_any: bool = false;
            for keyval in keys {
                if self.shard_identity.is_key_disposable(&keyval.key) {
                    tracing::debug!("Dropping ancestor key {}", keyval.key);
                    continue;
                }

                // TODO: we should accumulate consecutive stripes of keys and use vectored read
                let image_value = keyval
                    .val
                    .load(ctx)
                    .await
                    .context(format!("Loading key {}", keyval.key))?;

                written_any = true;
                image_layer_writer
                    .put_image(keyval.key, image_value, ctx)
                    .await
                    .context(format!("Storing key {}", keyval.key))?;
            }
